...
public static class GameOverPrep {
  static bool IsBorderBackground(Color c) {
    int max = Math.Max(c.R, Math.Max(c.G, c.B));
    int min = Math.Min(c.R, Math.Min(c.G, c.B));
    return max >= 188 && (max - min) <= 24;
  }
  ...
  public static void Prepare(string sourcePath, string targetPath) {
    using (var source = new Bitmap(sourcePath)) {
      ...
      Action<int, int> enqueue = (x, y) => {
        ...
        if (!IsBorderBackground(source.GetPixel(x, y))) return;
        visited[x, y] = true;
...
$sourcePath = (Resolve-Path (Join-Path $PSScriptRoot "..\sprites\gamover.jfif")).Path
$targetPath = Join-Path (Resolve-Path (Join-Path $PSScriptRoot "..\sprites")).Path "game-over-title.png"
[GameOverPrep]::Prepare($sourcePath, $targetPath)
